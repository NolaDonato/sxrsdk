...
RenderTarget* VulkanRenderer::createRenderTarget(Scene* scene, int w, int h)
{
    VkRenderTarget* renderTarget = new VkRenderTarget(scene, w, h);
    RenderSorter* sorter = new MainSceneSorter(0, true);
                                        ...
                                        RenderTexture* post_effect_render_texture_a, RenderTexture* post_effect_render_texture_b)
{
        ...
        (post_effects->pass_count() >= 0))
    {
        ...
        vulkanCore_->BuildCmdBufferForRenderData(render_data_list, rstate,
        ...
        for (int i = 0; i < postEffectCount-1; i++) {
            ...
            vulkanCore_->BuildCmdBufferForRenderDataPE(cmdbuffer, rstate, post_effects, renderTexture, i);
        ...
        if (!renderPostEffectData(rstate, input_texture, post_effects, postEffectCount - 1))
            return;
        vulkanCore_->BuildCmdBufferForRenderData(render_data_list, rstate, renderTarget, nullptr, true);
