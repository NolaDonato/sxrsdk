...
namespace sxr
{
    ...
    GLRenderer::GLRenderer(int token) : Renderer(token)
    {
    ...
    RenderTarget* GLRenderer::createRenderTarget(Scene* scene, int w, int h)
    {
        RenderTarget* renderTarget = new RenderTarget(scene, w, h);
        RenderSorter* sorter = new MainSceneSorter();
    ...
    ShadowMap* GLRenderer::createShadowMap(ShaderData* material)
    {
        ...
        {
            mShadowSorter = new ShadowRenderSorter(*material);
        }
                            ...
                            RenderTexture* post_effect_render_texture_b)
    {
        ...
        else
        {
            ...
            rstate.u_right = 0;
            if ((rstate.u_render_mask & RenderData::RenderMaskBit::Right) != 0)
            {
                rstate.u_right = 1;
            }
        }
