 ...
 * implementation corresponds to a subclass of SXRLight which is
 ...
 * shader.
 *
 * Each subclass of SXRLight is a different light implementation and has
 ...
 *
 * @see SXRShaderTemplate
 * @see SXRLight#setCastShadow(boolean)
 */
public class SXRLight extends SXRJavaComponent implements SXRDrawFrameListener
{
    ...
    protected SXRLight(SXRContext gvrContext, String uniformDesc, String vertexDesc)
    {
     ...
     * shader that performs the shadow map calculation. The built-in {@link SXRDirectLight}
     * and {@link SXRSpotLight} can produce shadows. {@link SXRPointLight} does not currently implement
     * shadow casting.
     *
    ...
    public void setOwnerObject(SXRNode newOwner)
    {
        ...
        if (newOwner != null)
        {
            if (owner == null)
            {
                ...
                if (mCastShadow)
                {
                    SXRShadowMap shadowMap = (SXRShadowMap) getComponent(SXRRenderTarget.getComponentType());
                    if (shadowMap == null)
                    {
                        setCastShadow(true);
                    }
     ...
     */
    public static SXRMaterial getShadowMaterial(SXRContext ctx)
    {
        return SXRShadowMap.getShadowMaterial(ctx);
    }
     ...
     * @see SXRShaderTemplate
     * @see SXRLight#getUniformDescriptor()
     *
     ...
     *
     * @see SXRShaderTemplate
     * @see SXRLight#getVertexDescriptor()
     *
     ...
     *
     * @see SXRShaderTemplate
     * @see SXRLight#getFragmentShaderSource()
     *
    ...
    static String makeShaderBlock(SXRScene scene)
    {
