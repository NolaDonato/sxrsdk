...
bool BulletURDFImporter::loadURDFString(const char* xmlstring, bool forceFixedBase)
{
    if (strlen(xmlstring) == 0)
        return false;
    BulletErrorLogger loggie;
    m_data->m_urdfParser.setParseSDF(false);
    return m_data->m_urdfParser.loadUrdf(xmlstring, &loggie, forceFixedBase, (m_data->m_flags & CUF_PARSE_SENSORS));
}
